...
装完可以先跑一次 doctor 验证环境就绪：

```bash
npm run doctor
```

全绿 ✓ 表示可以 `npm start`；有红 ✗ 按它提示的安装命令补齐。
...
**想先不配百度盘直接验证播放？** Admin 页还有"导入本地文件"按钮：
往 `library_path` 里丢一两个 `.mkv`，点这个按钮就会作为已缓存歌曲入库，
可以直接搜+点+播，跳过整个百度盘配置流程做烟雾测试。
